...
class BeautyProductVariant(BaseProductVariant):
    @property
    def link(self):
        return f"{self.raw_link}"


class BeautyProductPage(BaseProductPage):
    @property
    def link(self):
        return f"{PRODUCT_LIST_FE}?p={self.id}"


class BeautyProduct(BaseProduct):
    def __init__(
        self,
        source: str,
        product_id: str,
...
class BeautyScraper(BaseScraper):
    def __init__(
        self,
        run_id: str,
    ...
    def get_variant_volume(self, product: BeautyProduct) -> str:
        self.get(product.link)
    ...
    def get_variant_volume_unit(self, product: BeautyProduct) -> str:
        self.get(product.link)
        ...
        if match:
            ...
            logger.info("Không tìm thấy match unit")

    def get_variant_in_stock(self, product: BeautyProduct) -> int:
    ...
    def _load_product_variant(self, product: BeautyProduct):
    ...
    def get_numbers_of_page(self, base_url: str, max_pages = 100):
        page = 1
        while page <= max_pages:
            ...
            try:
                ...
                if button_expand.is_displayed():
                    logger.info("Have button so move to next page  %s", page + 1)
                    page += 1
                else:
                    logger.info(" No more button. Stop in page %s", page)
                    break
            except NoSuchElementException:
                logger.info("No more button. Stop in page %s, kết thúc.", page)
                break
        ...
        for page in range(1, max_page + 1):
            ...
            for product in products:
                try:
                    ...
                    full_url = self.get_product_link(product)
                    product_id = self.get_product_id(product)

                    product_obj = BeautyProduct(
                        source=HOMEPAGE,
                        product_id=product_id,
                        name=name,
                        brand=brand,
                        line="Parfum Femme" if gender == "Female" else "Parfum Homme",
                        gender=gender,
                        raw_link=full_url,
                    )
    ...
    def get_product_link(self, product):
        name_tag = product.select_one(LINK_PRODUCT_SELECTOR)
    ...
    def load_all_product(self):
        self.load_all_product_by_url(PRODUCT_LIST_FE, gender="Female")
        logger.info("Move to product male!")
        self.load_all_product_by_url(PRODUCT_LIST_MA, gender="Male")
